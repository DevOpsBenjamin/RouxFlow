...
#[test]
fn test_analyze_solve_3_debug() {
    let telemetry = test_data::solve_3();
    ...
    let trace_json = serde_json::to_string_pretty(&debug_trace).unwrap();
    std::fs::write("solve3_trace.json", trace_json).unwrap();

    let json = serde_json::to_string_pretty(&parsed).unwrap();
    std::fs::write("solve3.json", json).unwrap();

    let clean = parsed.to_clean();
    let clean_json = serde_json::to_string_pretty(&clean).unwrap();
    std::fs::write("solve3_clean.json", clean_json).unwrap();
}
