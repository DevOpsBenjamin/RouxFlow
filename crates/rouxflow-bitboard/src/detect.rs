...
impl BitCube {
    ...
    pub fn is_l_block_formed(&self) -> bool {
        if !self.is_uniform(Self::L_BLOCK) {
            return false;
        }
        if !self.is_uniform(Self::F_BAR_L) {
            return false;
        }
        if !self.is_uniform(Self::B_BAR_L) {
            return false;
        }
        if !self.is_uniform(Self::D_BAR_L) {
            return false;
        }
        true
    }
    ...
    pub fn is_cmll_solved(&self) -> bool {
        if !self
            .boards
            .iter()
            .any(|&b| b & Self::U_CORNERS == Self::U_CORNERS)
        {
            return false;
        }
        Self::SIDE_PAIRS
            .iter()
            .all(|&mask| self.boards.iter().any(|&b| b & mask == mask))
    }
    ...
    pub fn is_ul_ur_placed(&self) -> bool {
        ...
        self.get_color_at(3) == u_corner
            && self.get_color_at(37) == l_color
            && self.get_color_at(5) == u_corner
            && self.get_color_at(10) == r_color
    }
    ...
    pub fn is_l4e_solved(&self) -> bool {
        ...
        self.get_color_at(7) == u_c
            && self.get_color_at(19) == f_c
            && self.get_color_at(1) == u_c
            && self.get_color_at(46) == b_c
            && self.get_color_at(28) == d_c
            && self.get_color_at(25) == f_c
            && self.get_color_at(34) == d_c
            && self.get_color_at(52) == b_c
    }
    ...
    pub fn count_bad_edges(&self) -> usize {
        ...
        for (st1, st2) in edges {
            ...
            if Self::is_bad_edge(c1, c2, u_ref, f_ref) {
                bad += 1;
            }
        }
        bad
