...
<template>
  <div class="w-full h-full flex flex-col p-[2vmin] overflow-hidden gap-[2vh]">
    ...
    <!-- Bluetooth Required Overlay -->
    <Transition 
      enter-active-class="transition duration-700 ease-out"
      enter-from-class="opacity-0 translate-y-8"
      enter-to-class="opacity-100 translate-y-0"
    >
      <BluetoothRequired v-if="!bt.isConnected" />

      <!-- Actual Session View -->
      <div v-else class="flex-1 flex flex-col overflow-hidden gap-[2vh] animate-in fade-in duration-1000">
        <!-- Top Bar -->
